...
CASE_TAGS: tuple[tuple[str, str], ...] = (
    # ── Бизнес-вертикали BI Group (ось «Направление» в фильтрах кейсов).
    # Классификатор вешает их как обычные теги; фронтенд показывает эти шесть
    # отдельным дропдауном. «Финансы» и «HR» ниже в общем списке служат обеим осям.
    ("Строительство", "Стройплощадка, СМР, генподряд, объекты, прораб, технадзор, стройпроцессы"),
    ("Девелопмент", "Девелопмент недвижимости: продукт ЖК, продажи квартир, покупатели, земельный банк, проектирование"),
    ("Промышленность", "Производство, завод, промышленные предприятия, оборудование, цех, индустриальные проекты"),
    ("Корпоративный центр", "Штаб-квартира: корпоративное управление, стратегия группы, M&A, общие сервисы, политики"),
