...
class AnswerAgent(BaseAgent):
    ...
    async def answer_for_notebook(
        self,
        question: str,
        file_ids: list[str],
        language: str = "ru",
        file_id: str = "notebook-ask",
    ) -> AnswerResult:
        """Answer scoped to specific source files (notebook Q&A — LW-N17).

        Retrieval is restricted to chunks whose ``file_id`` is in *file_ids*.
        Returns a refusal when no chunks pass the similarity threshold.

        Args:
            question: User question.
            file_ids: Source document IDs attached to the notebook.
            language: Response language (``ru``, ``en``, or ``kk``).
            file_id: Correlation ID for usage logging.

        Returns:
            ``AnswerResult`` with answer, confidence, sources, and cost.
        """
        assert self._chunk_store is not None

        if not file_ids:
            return AnswerResult(
                answer=self._no_data_message(language),
                confidence="low",
                sources=[],
                cost_usd=0.0,
            )
